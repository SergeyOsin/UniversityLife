...
function Matrix = DoubleSimpleSolve(Matrix,minElem, minInd)
    ...
    if IndMinVal == 0
        error("Не найден разрешающий столбец (двойственный симплекс-метод).");
    end
...
function main()
    ...
    while (true)
       ...
       XI=zeros(1,rows-1);
       for i=1:rows-1
           if (abs(Matrix(i,cols) - round(Matrix(i,cols))) < 1e-9)
                  XI(i)=0;
           else
                  XI(i)=abs(Matrix(i,cols)-round(Matrix(i,cols)));
           end
       endfor
       disp("Xi="), disp(XI);
       count0=0;
       if max(XI) < 1e-9
          disp("Найдено целочисленное решение: ");
          break;
       endif
       [maxElem,maxInd]=max(XI);
       SelectedRow=Matrix(maxInd,:);
       for j = 1:cols
           val = SelectedRow(j);
           if abs(SelectedRow(j) - round(SelectedRow(j))) < 1e-9
               SelectedRow(j) = 0;
           else
               SelectedRow(j) = SelectedRow(j)-floor(SelectedRow(j));
           end
       endfor
       disp("Выбранная строка: ");
       disp(SelectedRow);
       if all(abs(SelectedRow(1:cols-1)) < 1e-9)
          error("Целочисленного решения нет");
       end
       SelectedRow=-SelectedRow;
       SelectedRow(end+1)=1;
       SelectedRow(end)=SelectedRow(end)-SelectedRow(end-1);
       SelectedRow(end-1)=SelectedRow(end)+SelectedRow(end-1);
       SelectedRow(end)=SelectedRow(end-1)-SelectedRow(end);
       disp("Дополнительное ограничение: "),
       disp(SelectedRow);

       DeletedCols=Matrix(:,cols);
       Matrix(:,cols)=[];
       Matrix=[Matrix, zeros(rows,1)];
       Matrix=[Matrix,DeletedCols];
       cols=cols+1;


       DeletedRow=Matrix(rows,:);
       Matrix(rows,:)=[];
       Matrix=[Matrix; SelectedRow];
       Matrix=[Matrix; DeletedRow];
       disp("Матрица после метода Гомери: ");
       disp(Matrix);
       NumIter=NumIter+1;
     end;
